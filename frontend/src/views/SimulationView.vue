...
const handleNextStep = (params = {}) => {
  ...
  // If custom rounds specified, pass via query params
  if (params.maxRounds) {
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // If graceful close failed, try force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Error closing simulation environment: ${closeErr.message}`)
        // If graceful close errored, try force stop
        await forceStopSimulation()
      }
    } else {
      // Environment not running, but process might still be active, check simulation status
    ...
    // Checking environment status failure doesn't affect subsequent flow
