            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content done, but section may not be complete) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show special prompt when it is the final answer -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep button in the same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/(?:Analysis Question|分析问题):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract prediction scenario
    const reqMatch = text.match(/(?:Prediction Scenario|预测场景):\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Extract statistics - match format
    const factMatch = text.match(/(?:Related Prediction Facts|相关预测事实):\s*(\d+)/)
    const entityMatch = text.match(/(?:Involved Entities|涉及实体):\s*(\d+)/)
    ...
    const subQSection = text.match(/### (?:Sub-questions Analyzed|分析的子问题)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/(?:Current Active Facts|当前有效事实):\s*(\d+)/)
    ...
    // Extract current active facts - full extraction, no limit
    const activeSection = text.match(/### (?:【Current Active Facts】|【当前有效事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    const entitySection = text.match(/### (?:【Involved Entities】|【涉及实体】)\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*(?:Interviewee Count|采访人数):\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### (?:Interviewee Selection Reasons|采访对象选择理由)\n([\s\S]*?)(?=\n---\n|\n### (?:Interview Transcripts|采访实录))/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format 1: digit. **name(index=X)**: reason
        // E.g.: 1. **alumni_345(index=1)**: As a Wuhan Univ alumni...
        ...
        } else if (currentName && line.trim() && !line.match(/^Not selected|^未选|^In summary|^综上|^Final selection|^最终选择/)) {
          // Continuation of reason (excluding ending summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compat: single platform marker case)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default if not placeholder text
          if (interview.redditAnswer && interview.redditAnswer !== '(No response received from this platform)' && interview.redditAnswer !== '（该平台未获得回复）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(No response received from this platform)' && interview.twitterAnswer !== '（该平台未获得回复）') {
      ...
      // Extract key quotes (support multiple quote formats)
    ...
    const summaryMatch = text.match(/### (?:Interview Summary and Key Insights|采访摘要与核心观点)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/(?:Found|找到)\s*(\d+)\s*(?:results|条)/)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response received from this platform)' || t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[No reply]' || t === '[无回复]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 2. "1. " or "\n1. " (number+period, legacy format)
      let matches = []
      let match

      // Prefer trying "QuestionX:" format
      const cnPattern = /(?:^|[\r\n]+)(?:Question|问题)(\d+)[：:]\s*/g
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons (only shown with real dual-platform answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 heading (## xxx), since section title is displayed in outer layer
  ...
  // Process line breaks - empty lines become paragraph separators, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <br> before paragraph opening tags after block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered list numbering: keep incrementing when single-item <ol> are separated by paragraph content
        ...
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Auto-expand freshly generated section
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown, try removing thinking process
