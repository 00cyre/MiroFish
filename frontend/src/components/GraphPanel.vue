    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still being processed. Consider refreshing the graph later.</span>
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to not simulating, show finish hint
...
const renderGraph = () => {
  ...
  // Track the current edge index for each node pair
  const edgePairIndex = {}
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on edge count between node pairs
      // Base distance 150, +40 for each additional edge
    ...
    // Add gravity toward center to cluster isolated node groups
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from node right side, loop around and back
    ...
    // Perpendicular offset based on distance ratio to ensure visible curves
