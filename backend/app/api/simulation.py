...
# Adding this prefix prevents the Agent from calling tools and makes it reply with text directly
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) are kept in the backend/scripts/ directory and are no longer copied to the simulation directory
    ...
    try:
        ...
        # - running: currently running, means preparation was completed earlier
        # - completed: run finished, means preparation was completed earlier
        # - stopped: stopped, means preparation was completed earlier
    ...
    try:
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Don't get edge info, to speed up
            )
...
def get_prepare_status():
    ...
    try:
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Preparation has not started, please call /api/simulation/prepare to begin",
                        "already_prepared": False
                    }
                })
            return jsonify({
        ...
        if not task:
            # Task not found, but if simulation_id exists, check if preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    and returns the most recent one (sorted by created_at) if there are multiple.
...
def get_simulation_history():
    ...
    try:
        ...
        # Enrich simulation data, only read from Simulation files
        ...
        for sim in simulations:
            ...
            # Get run state (read actual rounds set by user from run_state.json)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g., file modification time, whether generation is in progress)
    - Can return partial info even if configuration generation is not yet complete
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "config": {...}  // Configuration content (if available)
            }
        }
    """
    import json
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - generation_reasoning: LLM's configuration reasoning
    """
    try:
        manager = SimulationManager()
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "force": false                         // Optional: force restart (will stop running simulation and clean logs)
        }
    ...
    About the force parameter:
        ...
        - Suitable for scenarios where you need to re-run the simulation

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) will be updated to Zep graph in real-time
        - This allows the graph to "remember" the simulation process for subsequent analysis or AI chat
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is currently running. Please call /stop first, or use force=true to force restart"
                            }), 400
...
def get_run_status(simulation_id: str):
    """
    Get simulation real-time run status (for frontend polling)
...
def get_run_status_detail(simulation_id: str):
    """
    Get simulation detailed run status (including all actions)
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // Without platform, uses default
                }
            ],
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What is your overall view?",  // Required, interview question (same for all Agents)
