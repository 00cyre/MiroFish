...
Dynamically updates Agent activities from simulations to the Zep graph
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert an activity into a text description that can be sent to Zep
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation action log files and updates new agent activities
    to the Zep graph in real time. Groups by platform and sends to Zep
    in batches after accumulating BATCH_SIZE activities per platform.

    All meaningful actions are updated to Zep, with action_args containing
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Activity buffers grouped by platform (each platform accumulates to BATCH_SIZE before batch sending)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contains full context information (e.g., original post text, usernames, etc.).
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send activities to the Zep graph in batch (combined into a single text)
        ...
        # Combine multiple activities into a single text, separated by newlines
    ...
    def _flush_remaining(self):
        ...
        # First process remaining activities in the queue and add to buffers
