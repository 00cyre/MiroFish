...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # Create main log file to avoid process blocking from full stdout/stderr pipe buffers
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables to ensure UTF-8 encoding on Windows
            # This fixes issues with third-party libraries (e.g., OASIS) not specifying encoding when reading files
            env = os.environ.copy()
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group to ensure all related processes are terminated on server shutdown
            )
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform is running, only check that one
                                    # If both platforms are running, both must be complete
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Overall round takes the max of both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall time takes the max of both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        """
        Check if all enabled platforms have completed their simulations

        Determines platform enablement by checking if the corresponding actions.jsonl files exist
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        Args:
            ...
            timeout: Timeout in seconds for waiting process to exit
        """
        if IS_WINDOWS:
            # Windows: Use taskkill command to terminate process tree
            # /F = force kill, /T = terminate process tree (including child processes)
    ...
    def register_cleanup(cls):
        ...
        # If not in debug mode, this env var doesn't exist, and registration is also needed
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: Interview on both platforms in dual-platform mode, returning combined results
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: Interview on both platforms for each Agent in dual-platform mode
            timeout: Timeout in seconds
    ...
    def close_simulation_env(
        ...
        Send a close environment command to the simulation, allowing it to gracefully exit command-waiting mode
    ...
    def get_interview_history(
        ...
        Args:
            ...
            platform: Platform type (reddit/twitter/None)
                ...
                - None: Get history from all platforms
            agent_id: Specific Agent ID (optional, get history for this Agent only)
