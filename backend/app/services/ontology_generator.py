...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any natural person. When a person does not belong to a more specific person type, classify them here.
   - `Organization`: Fallback type for any organization. When an organization does not belong to a more specific organization type, classify it here.

B. **Specific types (8, designed based on text content)**:
   - Design more specific types for the main roles appearing in the text
   - For example: if the text involves academic events, you may have `Student`, `Professor`, `University`
   - For example: if the text involves business events, you may have `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Various figures will appear in the text, such as "elementary school teachers", "random passersby", "some netizen"
