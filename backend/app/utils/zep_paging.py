"""Zep Graph paginated fetching utility.

Zep's node/edge list APIs use UUID cursor-based pagination.
This module encapsulates automatic page-turning logic (with per-page retry),
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination, returning at most max_items entries (default 2000). Each page request includes retry logic."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges with pagination, returning the complete list. Each page request includes retry logic."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
