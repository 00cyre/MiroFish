...
# Add filter immediately at module load time to ensure it takes effect before camel code executes
logging.getLogger().addFilter(MaxTokensWarningFilter())


def disable_oasis_logging():
    """
    Disable verbose OASIS library log output
...
# Reddit available actions (excluding INTERVIEW, which can only be triggered manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing results, or dictionary containing error
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview on both platforms, return integrated results
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database with complete context information
    
    Args:
        db_path: Database file path
        last_rowid: Maximum rowid from last read (using rowid instead of created_at because different platforms have different created_at formats)
...
def _enrich_action_context(
    ...
    """
    Supplement action with context information (post content, usernames, etc.)
    ...
    try:
        ...
        # Create comment: add commented post information
        ...
        # Context enrichment failure does not affect main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM configuration for parallel simulation speedup:
...
async def run_twitter_simulation(
    ...
    # Note: do not close environment, keep it for Interview use
    ...
    log_info(f"Simulation loop completed! Time: {elapsed:.1f}s, total actions: {total_actions}")
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if available, otherwise falls back to general config)
...
async def main():
    ...
    # Create shutdown event at the start of main, ensuring the entire program can respond to shutdown signals
    ...
    # Initialize logging configuration (disable OASIS logs, clean old files)
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: do not exit after simulation, wait for interview commands
    When a termination signal is received, need to:
    1. Notify asyncio loop to exit waiting
